...
describe.each([{ permission: "Master" }, { permission: "Admin" }])(
  ...
  ({ permission }) => {
    ...
    test(`${permission} key: Update non separator tokens`, async () => {
      ...
      await client
        .index(index.uid)
        .setting.updateNonSeparatorTokens(newNonSeparatorTokens)
        .waitTask();
    ...
    test(`${permission} key: Reset NonSeparator tokens`, async () => {
      ...
      await client
        .index(index.uid)
        .setting.resetNonSeparatorTokens()
        .waitTask();
...
describe.each([
  ...
  test(`getNonSeparatorTokens route`, async () => {
    ...
    await expect(
      client.index(index.uid).setting.getNonSeparatorTokens(),
  ...
  test(`updateNonSeparatorTokens route`, async () => {
    ...
    const strippedHost = trailing ? host.slice(0, -1) : host;
    await expect(
      client.index(index.uid).setting.updateNonSeparatorTokens([]),
    ).rejects.toHaveProperty(
      "message",
      `Request to ${strippedHost}/${route} has failed`,
    );
  ...
  test(`resetNonSeparatorTokens route`, async () => {
    ...
    const strippedHost = trailing ? host.slice(0, -1) : host;
    await expect(
      client.index(index.uid).setting.resetNonSeparatorTokens(),
    ).rejects.toHaveProperty(
      "message",
      `Request to ${strippedHost}/${route} has failed`,
    );
