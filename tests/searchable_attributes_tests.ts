...
describe.each([
  ...
  test(`${permission} key: Get default searchable attributes`, async () => {
    await client.getIndex(index.uid).getSearchableAttributes()
  ...
  test(`${permission} key: Update searchable attributes`, async () => {
    const new_da = ['title']
    await client.getIndex(index.uid).updateSearchableAttributes(new_da)
  ...
  test(`${permission} key: Reset searchable attributes`, async () => {
    await client.getIndex(index.uid).resetSearchableAttributes()
      .then((response: Types.EnqueuedUpdate) => {
        expect(response).toHaveProperty('updateId', expect.any(Number))
        return response.updateId
      })
...
describe.each([
  ...
  test(`${permission} key: try to get searchable attributes and be denied`, async () => {
    await expect(client.getIndex(index.uid).getSearchableAttributes()).rejects.toThrowError(
      `Invalid API key: ${PUBLIC_KEY}`
    )
  })
  test(`${permission} key: try to update searchable attributes and be denied`, async () => {
    await expect(client.getIndex(index.uid).updateSearchableAttributes([])).rejects.toThrowError(
      `Invalid API key: ${PUBLIC_KEY}`
    )
  })
  test(`${permission} key: try to reset searchable attributes and be denied`, async () => {
    await expect(client.getIndex(index.uid).resetSearchableAttributes()).rejects.toThrowError(
      `Invalid API key: ${PUBLIC_KEY}`
    )
  })
})
